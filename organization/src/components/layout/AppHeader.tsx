...
export function AppHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-[68px] items-center justify-between border-b border-border/70 bg-background/80 px-4 backdrop-blur-xl sm:px-6 lg:px-8">
      ...
      {searchOpen && <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 px-4 pt-[12vh] backdrop-blur-sm" onMouseDown={() => setSearchOpen(false)}>
        <div className="w-full max-w-2xl overflow-hidden rounded-[1.5rem] border bg-card shadow-2xl" onMouseDown={e => e.stopPropagation()}>
          ...
          <div className="flex items-center gap-5 border-t bg-muted/40 px-4 py-2 text-[11px] text-muted-foreground"><span>↑↓ Navigate</span><span>↵ Open</span><span>Esc Close</span></div>
        </div>
      </div>}
    </header>
  );
