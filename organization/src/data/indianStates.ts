// Shared Indian state and district reference data.
// Used by the Create Branch address form and the Edit Branch dialog so both
// offer the same list instead of free-text entry.
...
export const DISTRICTS_BY_STATE: Record<string, string[]> = {
  ...
  "Punjab": ["Amritsar","Barnala","Bathinda","Faridkot","Fatehgarh Sahib","Fazilka","Ferozepur","Gurdaspur","Hoshiarpur","Jalandhar","Kapurthala","Ludhiana","Malerkotla","Mansa","Moga","Muktsar","Nawanshahr","Pathankot","Patiala","Rupnagar","Sahibzada Ajit Singh Nagar","Sangrur","Tarn Taran"],
...
export const toStateTitle = (value: string) =>
  value.replace(/-/g, " ").replace(/\s+/g, " ").trim().toLowerCase()
    .replace(/\b\w/g, (c) => c.toUpperCase());

/**
 * Map a stored state value onto its canonical entry in `INDIAN_STATES`, so a
 * slug or an odd casing still selects correctly. Unknown values pass through
 * unchanged rather than being silently dropped.
 */
/**
 * India Post still returns pre-rename and ampersand forms. Verified against
 * live PIN lookups, which is where these come from.
 */
...
export const canonicalState = (value: string) => {
  if (!value) return "";
  // "Jammu & Kashmir" and "Jammu and Kashmir" are the same state.
  const title = toStateTitle(value.replace(/&/g, " and "));
  const exact = INDIAN_STATES.find((s) => s.toLowerCase() === title.toLowerCase());
  if (exact) return exact;
  return STATE_ALIASES[title.toLowerCase()] ?? value;
};

/** Districts for a state, tolerating slugged or oddly cased state values. */
export const districtsFor = (state: string) =>
  DISTRICTS_BY_STATE[canonicalState(state)] ?? [];
