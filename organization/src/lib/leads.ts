...
/** The board: one column per stage, newest lead first inside each. */
export function pipeline(leads: AdmissionLead[]): Array<{ stage: LeadStage; leads: AdmissionLead[] }> {
 ...
 */
export function sourceBreakdown(leads: AdmissionLead[]): Array<{ source: string; count: number; percent: number }> {
...
export function monthlyEnquiries(
  leads: AdmissionLead[],
  months = 6,
  now = new Date(),
...
export function followUpQueue(leads: AdmissionLead[], now = new Date()): AdmissionLead[] {
  ...
  return leads
    ...
    .sort((a, b) => {
      ...
      return byBucket || new Date(a.followUpAt).getTime() - new Date(b.followUpAt).getTime();
    });
}
