...
type Auth={user:User|null;login:(identifier:string,password:string)=>Promise<void>;logout:()=>Promise<void>};
const Context=createContext<Auth|null>(null);
export function AuthProvider({children}:{children:React.ReactNode}){const [user,setUser]=useState<User|null>(()=>{try{return JSON.parse(localStorage.getItem("erp-user")||"null")}catch{return null}});
 ...
 const logout=async()=>{const refreshToken=localStorage.getItem("erp-refresh-token");try{await api("/auth/logout",{method:"POST",body:JSON.stringify({refreshToken})})}catch{}localStorage.removeItem("erp-access-token");localStorage.removeItem("erp-refresh-token");localStorage.removeItem("erp-user");setUser(null)};
